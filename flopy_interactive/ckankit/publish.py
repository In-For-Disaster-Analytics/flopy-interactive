...
def _now_iso() -> str:
    ...
    Returns:
        ISO-8601 timestamp string in UTC (with trailing ``Z``).
    """
...
def _slugify(value: str) -> str:
    ...
    Returns:
        Slugified string suitable for CKAN names.
    """
...
def _extras_to_dict(extras: Iterable[Dict]) -> Dict[str, str]:
    ...
    Args:
        extras: Iterable of CKAN extras dicts.

    Returns:
        Dict of extras keys to string values.
    """
    result: Dict[str, str] = {}
...
def _merge_extras(existing: Iterable[Dict], additions: Dict[str, str]) -> List[Dict]:
    ...
    Args:
        existing: Existing extras list.
        additions: Extra keys to add/overwrite.

    Returns:
        CKAN extras list-of-dict structure.
    """
...
def _extract_mint_svo(extras: Iterable[Dict]) -> Optional[str]:
    ...
    Args:
        extras: CKAN extras list.

    Returns:
        Standard variable string or None if not found.
    """
    candidates = [
        "MINT_SVO",
...
def _resolve_mint_svo(resource: Dict, dataset: Dict, target_var: str) -> Optional[str]:
    ...
    Args:
        ...
        dataset: CKAN dataset metadata dict.
...
def package_show(jwt_token: str, dataset_name: str) -> Dict:
    ...
    Args:
        jwt_token: JWT token string.
        dataset_name: CKAN dataset name or id.
...
def create_dataset(jwt_token: str, dataset_dict: Dict) -> Dict:
    ...
    Args:
        jwt_token: JWT token string.
        dataset_dict: Dataset payload dict.

    Returns:
        Created dataset metadata dict.
    """
...
def create_resource_upload(
    ...
    Args:
        ...
        resource_dict: Resource payload dict.

    Returns:
        Created resource metadata dict.
    """
...
def build_dataset_payload(
    ...
    Args:
        source_dataset: Source dataset metadata dict.
        new_name: New dataset name.
        provenance: Provenance metadata dict.
        mint_svo: Standard variable string.
        source_url: Optional source URL string.
        change_summary: Optional change summary string.
        maintainer_username: Optional maintainer username.

    Returns:
        Dataset payload dict for CKAN.
    """
    copy_fields = [
        "title",
        "notes",
...
def build_resource_payload(
    ...
    Args:
        source_resource: Source resource metadata dict.
        provenance: Provenance metadata dict.
        mint_svo: Standard variable string.
        source_url: Optional source URL string.
        change_summary: Optional change summary string.
    ...
    Returns:
        Resource payload dict for CKAN.
    """
...
def publish_updated_wel(
    ...
    Args:
        ...
        output_path: Path to the updated WEL file.
    ...
    wel_resource = next(
        (res for res in resources if resource_has_standard_var(res, WEL_STANDARD_VAR)),
        None,
    )
    if not wel_resource:
